...
{
    public class W6092FeedConstant : Message<uint>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 11;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x02;
        byte byteIndexLB = 0x92;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[13];

        public W6092FeedConstant(uint val, byte subIndex) : base(0x6092, subIndex, val)
        {
        }

        public W6092FeedConstant(ISerialClient receiver, uint value, byte subIndex) : base(receiver, 0x6092, subIndex)
        {
            message[0] = byteSOF;
    ...
    public static class W6092FeedConstantSubIndexes
    {
        public const byte Feed = 0x01;
        public const byte ShaftRevolutions = 0x02;
    }
}
