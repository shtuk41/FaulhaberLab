...
{
    public class W609AHomingAcceleration : Message<uint>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 11;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x02;
        byte byteIndexLB = 0x9A;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[13];

        public W609AHomingAcceleration(ISerialClient receiver, uint value) : base(receiver, 0x609A, 0x00)
        {
            message[0] = byteSOF;
