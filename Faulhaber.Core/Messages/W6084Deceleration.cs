...
{
    public class W6084Deceleration : Message<uint>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 11;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x02;
        byte byteIndexLB = 0x84;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[13];

        public W6084Deceleration(uint val) : base(0x6084, 0x00, val)
        {
        }

        public W6084Deceleration(ISerialClient receiver, uint value) : base(receiver, 0x6084, 0x00)
        {
            message[0] = byteSOF;
