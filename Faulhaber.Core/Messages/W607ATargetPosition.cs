...
{
    public class W607ATargetPosition : Message<int>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 11;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x02;
        byte byteIndexLB = 0x7A;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[13];

        public W607ATargetPosition(int val) : base(0x607A, 0x00, val)
        {
        }

        public W607ATargetPosition(ISerialClient receiver, int value) : base(receiver, 0x607A, 0x00)
        {
            message[0] = byteSOF;
