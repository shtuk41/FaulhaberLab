...
{
    public class W6080MaximumSpeed : Message<uint>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 11;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x02;
        byte byteIndexLB = 0x80;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[13];

        public W6080MaximumSpeed(uint val) : base(0x6080, 0x00, val)
        {
        }

        public W6080MaximumSpeed(ISerialClient receiver, uint value) : base(receiver, 0x6080, 0x00)
        {
            message[0] = byteSOF;
            ...
            message[9] = (byte)((value & 0x00FF0000) >> 16);
