...
{
    public class W6064PositionActualValue : Message<int>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 7;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x01;
        byte byteIndexLB = 0x64;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[9];

        public W6064PositionActualValue(int val) : base(0x6064, 0x00, val)
        {
        }

        public W6064PositionActualValue(ISerialClient receiver) : base(receiver, 0x6064, 0x00)
        {
            message[0] = byteSOF;
