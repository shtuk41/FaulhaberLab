...
{
    public class W6062PositionDemandValue : Message<int>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 7;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x01;
        byte byteIndexLB = 0x62;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[9];

        public W6062PositionDemandValue(int val) : base(0x6062, 0x00, val)
        {
        }

        public W6062PositionDemandValue(ISerialClient receiver) : base(receiver, 0x6062, 0x00)
        {
            message[0] = byteSOF;
