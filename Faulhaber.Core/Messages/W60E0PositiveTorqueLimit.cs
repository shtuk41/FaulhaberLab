...
{
    public class W60E0PositiveTorqueLimit : Message<ushort>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 9;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x02;
        byte byteIndexLB = 0xE0;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[11];

        public W60E0PositiveTorqueLimit(ushort val) : base(0x60E0, 0x00, val)
        {
        }

        public W60E0PositiveTorqueLimit(ISerialClient receiver, ushort val) : base(receiver, 0x60E0, 0)
        {
            message[0] = byteSOF;
