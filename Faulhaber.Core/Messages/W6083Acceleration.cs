...
{
    public class W6083Acceleration : Message<uint>
    {
        byte byteSOF = (byte)'S';
        byte byteUserDataLength = 11;
        byte byteNodeNumber = 0;
        byte byteCommandCode = 0x02;
        byte byteIndexLB = 0x83;
        byte byteIndexHB = 0x60;
        byte byteEOF = (byte)'E';

        byte[] message = new byte[13];

        public W6083Acceleration(uint val) : base(0x6083, 0x00, val)
        {
        }

        public W6083Acceleration(ISerialClient receiver, uint value) : base(receiver, 0x6083, 0x00)
        {
            message[0] = byteSOF;
